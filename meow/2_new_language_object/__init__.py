...
import srsly
from .lemmatizer import MeowLemmatizer

# https://nightly.spacy.io/api/language#defaults
class MeowDefaults(Language.Defaults):
...
@spacy.registry.languages("meow") #https://nightly.spacy.io/api/top-level#registry
class Meow(Language):
    lang = "meow"
    Defaults = MeowDefaults
...
def make_lemmatizer(
    ...
    return MeowLemmatizer(nlp.vocab, model, name, mode=mode, overwrite=overwrite)

#Add locations of lookups data to the registry
@spacy.registry.lookups("meow")
def do_registration():
